...
function fetchTwoVideos(done) {
    return fetch("https://k6y3d3jhhe.execute-api.us-east-2.amazonaws.com/prod/get_video_pairs")
    .then(r => {
        if (!r.ok) throw new Error('No pair of videos available');
        return r.json();
    })
    .then(json => {
        Object.assign(nextTwoVideos, json);
        console.log('Fetched videoData:', nextTwoVideos);
        done();  
    })
    .catch(err => {
        console.error('Error fetching next video:', err);
        nextTwoVideos.videos = [];
        done();
    });
}
